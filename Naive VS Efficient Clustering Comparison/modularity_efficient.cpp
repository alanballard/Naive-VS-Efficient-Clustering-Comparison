...
)
{
	...
	{
		cout << "EFFICIENT MODULARITY METHOD, k=" << k << endl;
		...
		//Step 1: Initialize kxk current/proposed OBS and POS matrices, and current/proposed membership vector
		...
		while (IT > LimitIT)
		{
			...
			while (first_run == 1)
			{
				//Step 2: Make initial membership
				...
				//Step 3: Calculate modularity of current membership 
				//Don't need...
			...
			//Enter SA Algorithm
//Step 4: Propose a new clustering by mutating current clustering
...
//Step 5: Calculate modularity of the proposed membership
//Don't need...
//Step 6: Calculate change in modularity due to reclustering
//			delta_mod = mod_1 - mod_0;
//Can calculate delta_mod (i.e. 'delta Q') immediately, without explicitely calculating modularity of currently/proposed clustering
//by using the formula for 'delta Q' found in the so-called Louvain Method.
//'Fast unfolding of communities in large networks': https://arxiv.org/pdf/0803.0476.pdf
//Also: https://www.quora.com/How-is-the-formula-for-Louvain-modularity-change-derived
			//Note: the original paper refers to weight of edges but in these Binomial networks, all weights=1 so sum(weights)=count
			int SUM_in = 0;//Count of distinct edges between vertices of clust_j
			int SUM_tot = 0; //Count of distinct edges between vertices of clust_j and ALL other vertices
			int k_i_in = 0; //Count of distinct edges between l_star and vertices of clust_j
			int k_i = 0; // Count of distinct edges between l_star and ALL other vertices
			//m: # of distict edges in network = nb_links in this code

			//Calculate: SUM_tot, SUM_in, k_i_in
			for (int i = 0; i < links.size(); i++)
			{
				//if vertex i is in clust_j, continue
				if (proposed_membership[i] == clust_j)
				{
					SUM_tot = SUM_tot + links[i].size(); //Add vertex i's degree to the total count of clust_j vertex degrees

					//Check vertex i's neighbors
					for (int j = 0; j < links[i].size(); j++)
					{

						if (proposed_membership[links[i][j].first] == clust_j) //both vertices i and j are members of clust_j
						{
							SUM_in = SUM_in + 1; //Add to total count of edges between members of clust_j

							if (i == l_star) //get count of edges between vertex l_star and members of clust_j
							{
								k_i_in = k_i_in + 1;
							}
						}
			...
			//Each distinct edge is represented twice in adjacency list. Divide by 2 to get true edge count.
			SUM_in = SUM_in / 2;
			k_i_in = k_i_in / 2;
			SUM_tot = SUM_tot / 2;

			k_i = links[l_star].size();
			//cout << SUM_in << "..." << k_i_in << "..." << SUM_tot << "..." << k_i << endl;

			long double part_1 = (SUM_in + 2 * k_i_in) / (long double)(2 * nb_links);
			long double part_2 = ((SUM_tot + k_i) / (long double)(2 * nb_links)) * ((SUM_tot + k_i) / (long double)(2 * nb_links));
			long double part_3 = (SUM_in) / (long double)(2 * nb_links);
			long double part_4 = ((SUM_tot) / (long double)(2 * nb_links)) * ((SUM_tot) / (long double)(2 * nb_links));
			long double part_5 = (k_i) / (long double)(2 * nb_links) * (k_i) / (long double)(2 * nb_links);
			delta_mod = (part_1 - part_2) - (part_3 - part_4 - part_5);
			//cout << part_1 << "..." << part_2 << "..." << part_3 << "..." << part_4 << "..." << part_5 << "..." << delta_mod << endl;
			//cin.get();
			...
			/*
						...
						long double check_mod_0 = e_mm - a_mm; //modularity of current membership assignment
						...
						for (int i = 0; i < links.size(); i++)
						{
							for (int j = 0; j < links[i].size(); j++)
							{
								...
								{
									e_mm = e_mm + 1; //counting how many edges fall between vertices within the same cluster
									a_m[proposed_membership[i] - 1] = a_m[proposed_membership[i] - 1] + 1; //counting how many edges fall between vertices within cluster i
								}
								//between cluster edges
								else
								{
									//a_m[i-1] contains the number of edges between vertices of cluster i and other clusters (but not between vertices of cluster i)
									a_m[proposed_membership[i] - 1] = a_m[proposed_membership[i] - 1] + 1; //counting how many edges fall between vertices of cluster i and other clusters
								}
							}
						}
						//Note: Since the adjacency list contains 2 entries for every edge and the number of observed edges are counted
						...
						long double check_mod_1 = e_mm - a_mm; //modularity of proposed membership assignment
			cout<<"check_mod_0: "<<check_mod_0<<", check_mod_1: "<<check_mod_1<<endl;
			cout << "true change in Q is: " << check_mod_1 - check_mod_0 << endl;
			cout << "delta Q is: " << delta_mod << endl;
			check_mod_0 = 0;
			check_mod_1 = 0;
			cin.get();
			*/
			...
			{//ACCEPT PROPOSED CLUSTERING AS CURRENT (best) CLUSTERING
			 //Compare the two modularities. If accept new mod, then:
			 ...
			 //Proposed mod becomes current mod. Don't need to repeat step 4.		
				STAGE = 1;
			...
			else
			{
				...
				double uni_draw = rand() / double(RAND_MAX);//dist_uni(engine)/*uniform_rand(rand())*/;
				long double min_val = min(one, exp(delta_mod / IT));

				//cout << "mod_0="<<mod_0<<" mod_1="<<mod_1<<" delta_mod="<<delta_mod<< " uni_draw="<<uni_draw<<" exp(blah)="<<min_val << endl; 
...
//Since we are only calculating change-in-modularity for each proposed solution, we now need to calculate the full modularity score for the final solution clustering
	...
	// e_mm=raction of edges that connect community m to community m
	// a_i=the fraction of edges that connect to vertices in community i
to vertices in community i
	Q = Q + (e_mm - a_m^2);
	*/
...
//In the case of modularity, we need to populate the OBS/POS matrices for the final membership here in order to calculate the likelihood fo the modularity solution, so we can calculate BIC.
		for (int i = 0; i < links.size(); i++)
		{
			for (int j = 0; j < links[i].size(); j++)
			{
...
//Therefore the BIC, loglikelihood and test statistic for the solution clustering under modularity are not particularly meaningful. Just calculating here to be thorough.
		...
		cout << "at k=" << k << ", modularity= " << mod_0 << ", loglik=" << full_loglik_curr << " and BIC=" << BIC << endl;
		...
		//remove("completion_mod_new_all_sols.txt");
		atkstring << "completion_mod_new_all_sols" << ".txt";
		atkinfo.open("completion_mod_new_all_sols.txt", ios::app);
		atkinfo << filename << ", " << N << ", " << nb_links << ", " << k << "," << mod_0 << ", " << BIC << ", " << TESTSTAT << ", " << time << endl;
		atkinfo.close();

		//cout << "at k=" << k << ", modularity= " << mod_0 << endl;;
	}//end for-k loop

	ostringstream atkstring2;
	ofstream atkinfo2;
	// If old file exists from a previous run, delete it.
	//remove("completion_mod_new_final_sol.txt");
	atkstring2 << "completion_mod_new_final_sol" << ".txt";
	atkinfo2.open("completion_mod_new_final_sol.txt", ios::app);
	atkinfo2 << filename << ", " << N << ", " << nb_links << ", " << final_k << "," << final_MODULARITY << ", " << final_BIC << ", " << final_TESTSTAT;
	atkinfo2.close();
	...
	// If old file exists from a previous run, delete it.
	//remove("solution_mod_new.txt");
	bestzsol << "solution_mod_new" << ".txt";
	bestzstorage.open("solution_mod_new.txt", ios::app);
